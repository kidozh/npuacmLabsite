        ...
        if len(password) <= 8 or len(password) >= 21:
            ...
            if admin.select().where(admin.username == username).count():
                ...
                if aimAdmin.authPassword(password):
                    ...
                    # redirect
                    self.redirect(self.reverse_url('contrib.admin.view.adminManageHandler'))
                else:
...
class appModelManager(BaseHandler):
    ...
    def getValidFieldByModel(self,model):
        ...
        :param model: model instance which inherit baseModel
        :return: all valid field in peewee or playhouse
        '''
        from db.appModel import fieldFinder
    ...
    def get(self, modelName,*args, **kwargs):
        ...
        allFieldName = [getattr(aimModel,foo) for (foo,instance) in allValidField]
        # set up query
...
class appModelAddManager(BaseHandler):
    def getValidFieldByModel(self,model):
        '''
        get all valid field in peewee or playhouse
        :param model: model instance which inherit baseModel
    ...
    def get(self, modelName,*args, **kwargs):
        ...
        allValidField = self.getValidFieldByModel(aimModel)
        allFieldName = [getattr(aimModel,fieldName) for (fieldName,field) in allValidField]
        ...
        try:
            for key in self.request.arguments:
                ...
                if hasattr(modelRec,key) and self.get_argument(key,0):
                    # print key,self.get_argument(key),type(getattr(modelRec,key)) == bool
                    if type(getattr(modelRec,key)) == bool and self.get_argument(key) == '1':

                        setattr(modelRec,key,True)
                    elif type(getattr(modelRec,key)) == bool and self.get_argument(key) == '0':
            ...
            self.write_error(500)
            return
        self.redirect(self.reverse_url('contrib.admin.view.appModelManager',modelName))
...
class appModelChangeManager(BaseHandler):
    def getValidFieldByModel(self,model):
        '''
        get all valid field in peewee or playhouse
        :param model: model instance which inherit baseModel
    ...
    def post(self, modelName,id,*args, **kwargs):
        import peewee,playhouse.fields
        ...
        for key in self.request.arguments:
            if hasattr(aimRow,key) and self.get_argument(key,None):
                ...
                continue
        aimRow.save()
        self.redirect(self.application.reverse_url('contrib.admin.view.appModelManager',modelName))

class appModelDeleteManager(BaseHandler):
    def getValidFieldByModel(self,model):
        '''
        get all valid field in peewee or playhouse
        :param model: model instance which inherit baseModel
...
class appModelApiManager(BaseHandler):
    '''
    ...
    @tornado.web.authenticated
    def post(self,modelName, *args, **kwargs):
        # get model's name from URL directly
        package,aimModel = self.getModelByName(modelName)
        if aimModel == None:
            self._reason = '您所请求的模型不存在或者并未被注册'
            self._status_code = 404
        ...
        while success :
            ...
            try :
                ...
                columnsInfo.append((data,name,searchable,orderable,searchval,searchRegex))
            except Exception as e:
                success = False
            finally:
                i +=1
        ...
        allNeededField = [getattr(aimModel,name) for (data,name,searchable,orderable,searchVal,searchRegex) in columnsInfo ]
        ...
        if searchVal :
            ...
            searchRequirement = [getattr(aimModel,name).contains(searchVal)
                                 for (data,name,searchable,orderable,i,searchRegex) in columnsInfo
                                 if searchable and (isinstance(getattr(aimModel,name), TextField) or isinstance(getattr(aimModel,name),CharField))]
            ...
            if len(searchRequirement) >0:
                ...
                # print len(aimModel.select(*allNeededField).where(getattr(aimModel,'codeA').contains('%s'%(searchVal))))
                query = aimModel.select(*allNeededField).order_by(*orderField).where(limit)[start:start+length]
                recordsFiltered = aimModel.select(*allNeededField).order_by(*orderField).where(limit).count()
            else:
                query = aimModel.select(*allNeededField).order_by(*orderField)[start:start+length]
                recordsFiltered = aimModel.select(*allNeededField).order_by(*orderField).count()
        else:
            query = aimModel.select(*allNeededField).order_by(*orderField)[start:start+length]
            recordsFiltered = aimModel.select(*allNeededField).order_by(*orderField).count()
        # assembly the result
